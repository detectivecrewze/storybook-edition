(function (root, factory) {
  ...
  const MODULE_TYPES = Object.freeze(["reasons", "gallery", "music", "letter"]);
  const PROJECT_ID_PATTERN = /^[a-z0-9][a-z0-9-]{2,63}$/;

  const UI_DEFAULTS = Object.freeze({
    id: Object.freeze({
      moduleTitles: Object.freeze({ reasons: "Kenapa Kamu Berarti", gallery: "Arsip Kenangan", music: "Soundtrack Kita", letter: "Sebuah Surat" }),
      moduleSubtitles: Object.freeze({ reasons: "Hal-hal kecil yang membuatmu istimewa.", gallery: "Momen yang ingin selalu disimpan.", music: "Lagu-lagu yang membawa kita kembali.", letter: "Kata-kata yang ingin kusampaikan." }),
    ...
    en: Object.freeze({
      moduleTitles: Object.freeze({ reasons: "Why You Matter", gallery: "Memory Archive", music: "Our Soundtrack", letter: "A Letter" }),
      moduleSubtitles: Object.freeze({ reasons: "The little things that make you special.", gallery: "Moments worth keeping forever.", music: "Songs that bring us back.", letter: "Words I have been meaning to say." }),
  ...
  const VALIDATION_MESSAGES = Object.freeze({
    id: Object.freeze({ projectId: "Project ID tidak valid.", recipient: "Nama penerima wajib diisi.", sender: "Nama pengirim wajib diisi.", modules: "Aktifkan setidaknya dua bagian gift.", reasons: `Isi ${MIN_REASONS}–${MAX_REASONS} alasan.`, gallery: "Tambahkan setidaknya satu foto atau video.", music: "Setiap lagu aktif membutuhkan file dan judul.", letter: "Lengkapi greeting, isi, dan penutup surat." }),
    en: Object.freeze({ projectId: "The project ID is invalid.", recipient: "Enter the recipient name.", sender: "Enter the sender name.", modules: "Enable at least two gift sections.", reasons: `Write ${MIN_REASONS}–${MAX_REASONS} reasons.`, gallery: "Add at least one photo or video.", music: "Every active song needs an audio file and title.", letter: "Complete the letter greeting, body, and signoff." })
  });
  ...
  const OCCASION_PRESETS = Object.freeze({
    ...
    birthday: Object.freeze({ id: "birthday", label: Object.freeze({ id: "Ulang Tahun", en: "Birthday" }), copy: Object.freeze({ id: Object.freeze({ eyebrow: "Hari spesialmu", title: "Selamat ulang tahun, bintang utama hari ini", message: "Ada empat kejutan kecil yang dibuat khusus untuk merayakanmu." }), en: Object.freeze({ eyebrow: "Your special day", title: "Happy birthday to today’s brightest star", message: "Four little surprises are waiting here to celebrate you." }) }) }),
  ...
  function moduleDefaults(language) { const locale = UI_DEFAULTS[normalizeLanguage(language)]; return MODULE_TYPES.map((type, order) => ({ type, enabled: true, order, title: locale.moduleTitles[type], subtitle: locale.moduleSubtitles[type] })); }
  ...
  function emptyProject(projectId = "new-storybook", language = "id") {
    ...
    return {
      ...
      identity: { recipient: "", sender: "", eventDate: "" }, opening: { ...opening }, modules: moduleDefaults(locale), reasons: { items: defaultReasons(locale) },
      gallery: { items: [{ id: makeId("media"), mediaType: "image", mediaUrl: "", title: "", caption: "" }] }, music: { tracks: [] }, letter: { greeting: "", paragraphs: [], signoff: "" },
  ...
  function normalizeModules(source, language) {
    ...
    return MODULE_TYPES.map((type, fallbackOrder) => {
      const item = incoming.find(candidate => candidate?.type === type) || {};
      const fallback = defaults[fallbackOrder];
      return { type, enabled: item.enabled !== false, order: Number.isFinite(Number(item.order)) ? Number(item.order) : fallbackOrder, title: text(item.title, fallback.title, 80), subtitle: text(item.subtitle, fallback.subtitle, 160) };
  ...
  function normalizeProject(input, projectId, existing = null) {
    ...
    return {
      ...
      opening: { eyebrow: text(source.opening?.eyebrow, fallback.opening.eyebrow, 100), title: text(source.opening?.title, fallback.opening.title, 180), message: text(source.opening?.message, fallback.opening.message, 400) },
  ...
  function applyOccasionPreset(project, presetId, language = project?.settings?.language) { const normalized = normalizeProject(project, project?.projectId); normalized.occasionPreset = normalizeOccasion(presetId); normalized.opening = { ...presetCopy(normalized.occasionPreset, normalizeLanguage(language)) }; return normalized; }
