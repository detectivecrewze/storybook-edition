(function (root) {
  ...
  class StorybookApi {
    ...
    upload(file, kind) {
      ...
      return jsonRequest("/api/upload", { method: "POST", headers: tokenHeaders(this.token), body, timeoutMs: 60000 }).then(result => { console.info("[Storybook Studio] Upload completed", { projectId: this.projectId, kind, mock: false }); return result; }).catch(error => { console.error("[Storybook Studio] Upload failed", { projectId: this.projectId, kind, status: error?.status, message: error?.message }, error); throw error; });
    }
  }
