(function () {
  ...
  async function initialize() {
    ...
    try { const payload = await api.getStudio(); draft = Project.normalizeProject(payload.project, projectId); giftUrl = payload.giftUrl || `${location.origin}/gift/${projectId}`; published = draft.status === "published"; catalog = await fetch("/assets/data/music.json").then(response => response.ok ? response.json() : []).catch(() => []); bind(); $("#gift-preview").src = `/gift/${encodeURIComponent(projectId)}?preview=1`; renderFields(); $("#studio-state").hidden = true; $("#studio-app").hidden = false; saveIndicator("saved"); } catch (error) { setState("Studio belum bisa dibuka", error.message, true); }
  }
  initialize();
})();
