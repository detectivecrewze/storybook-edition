(function () {
  ...
  function saveDraft() {
    ...
    saveQueue = saveQueue.catch(() => {}).then(() => api.saveStudio(draft, "draft")).then(result => { draft = Project.normalizeProject(result.project || draft, projectId, draft); saveIndicator("saved"); }).catch(error => { saveIndicator("dirty"); console.error("[Storybook Studio] Draft save failed", { projectId, status: error?.status, message: error?.message }, error); });
    return saveQueue;
  }
  ...
  async function uploadOpeningPanel(file, index) { if (!file) return; if (file.size > 8 * 1024 * 1024) return alert(I18n.t("studio.maxImage")); try { console.info("[Storybook Studio] Preparing opening panel", { projectId, panel: index + 1, size: file.size, type: file.type }); const result = await api.upload(await imageWebpPreserve(file), "photo"); draft.opening.panelImages[index] = result.url; renderOpeningPanels(); queueSave(); } catch (error) { reportUploadError("Foto opening", error); renderOpeningPanels(); } }
  async function uploadGallery(file, item, card) { if (!file) return; const isVideo = file.type.startsWith("video/"); const max = isVideo ? 20 * 1024 * 1024 : 8 * 1024 * 1024; if (file.size > max) return alert(I18n.t(isVideo ? "studio.maxVideo" : "studio.maxImage")); const label = $(".upload-button", card); label.textContent = "Uploading..."; try { console.info("[Storybook Studio] Preparing gallery media", { projectId, mediaType: isVideo ? "video" : "photo", size: file.size, type: file.type }); const uploadFile = isVideo ? file : await imageWebp(file); const result = await api.upload(uploadFile, isVideo ? "video" : "photo"); item.mediaType = isVideo ? "video" : "image"; item.mediaUrl = result.url; renderGallery(); queueSave(); } catch (error) { reportUploadError(isVideo ? "Video galeri" : "Foto galeri", error); renderGallery(); } }
  async function uploadAtlasPhoto(file, location) { if (!file) return; if (file.size > 8 * 1024 * 1024) return alert(I18n.t("studio.maxImage")); try { console.info("[Storybook Studio] Preparing map photo", { projectId, size: file.size, type: file.type }); const result = await api.upload(await imageWebp(file), "photo"); location.photoUrl = result.url; renderAtlas(); queueSave(); } catch (error) { reportUploadError("Foto lokasi", error); renderAtlas(); } }
